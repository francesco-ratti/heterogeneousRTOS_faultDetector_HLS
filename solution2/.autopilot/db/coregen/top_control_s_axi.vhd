...
architecture behave of top_control_s_axi is
    ...
    component top_control_s_axi_ram is
        generic (
    ...
    end component top_control_s_axi_ram;
...
-- ----------------------- Instantiation------------------
-- int_errorInTask
int_errorInTask : top_control_s_axi_ram
generic map (
     MEM_STYLE => "auto",
     MEM_TYPE  => "T2P",
     BYTES     => 4,
...
-- int_outcomeInRam
int_outcomeInRam : top_control_s_axi_ram
generic map (
     MEM_STYLE => "auto",
     MEM_TYPE  => "S2P",
     BYTES     => 36,
...
-- ----------------------- Register logic ----------------
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                int_failedTask <= (others => '0');
            elsif (ACLK_EN = '1') then
                if (failedTask_ap_vld = '1') then
                    int_failedTask <= UNSIGNED(failedTask);
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                ...
                elsif (ar_hs = '1' and raddr = ADDR_FAILEDTASK_CTRL) then
                    int_failedTask_ap_vld <= '0'; -- clear on read
                end if;
            end if;
        end if;
    end process;
...
entity top_control_s_axi_ram is
    generic (
        MEM_STYLE : STRING :="auto";
        MEM_TYPE  : STRING :="S2P";
        BYTES   : INTEGER :=4;
...
end entity top_control_s_axi_ram;

architecture behave of top_control_s_axi_ram is
