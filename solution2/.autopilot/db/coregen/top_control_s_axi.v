...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (failedTask_ap_vld)
            int_failedTask <= failedTask;
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        else if (ar_hs && raddr == ADDR_FAILEDTASK_CTRL)
            int_failedTask_ap_vld <= 1'b0; // clear on read
    end
end
