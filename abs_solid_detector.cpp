...
int find_region(region_t regions[MAX_REGIONS], float d[MAX_AOV_DIM]){
	int idx = -1;
	float score = -1;
...
void insert_point(region_t regions[MAX_REGIONS], int &n_regions, float d[MAX_AOV_DIM], bool is_accept){
...
bool run_train_sw(region_t regions[MAX_REGIONS], int &n_regions, float data[MAX_AOV_DIM], float inputData[MAX_AOV_DIM]){
...
bool run_test_sw(region_t regions[MAX_REGIONS], float data[MAX_AOV_DIM]) {
	return !( !is_valid(data) || find_region(regions, data) < 0 ) ;
}
...
void run_test(bool &error, region_t regions[MAX_REGIONS], float data[MAX_AOV_DIM]) {
#pragma HLS PIPELINE II=8
	error = ( !is_valid(data) || find_region(regions, data) < 0 ) ;
}

void runTestAfterInit(float * inputDataInRam, ap_int<16> taskId, OutcomeStr* outcomeInRam, hls::stream< ap_int<8> > &toScheduler, float data[MAX_CHECKS][MAX_AOV_DIM], region_t regions[MAX_CHECKS][MAX_REGIONS]) {
#pragma HLS dataflow
	...
	run_test(error, regions[taskId], data[taskId]);
...
#pragma HLS PIPELINE off
		...
		runTestAfterInit(inputDataInRam, contr.taskId, outcomeInRam, toScheduler, data, regions);
	}
}
//}
