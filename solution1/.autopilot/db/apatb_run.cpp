...
long __xlx_apatb_param_toScheduler_stream_buf_final_size;
{
    ...
    } // end post check logic bolck
  {
      ...
      if (rtl_tv_out_file.good()) {
        ...
        if (atoi(AESL_num.c_str()) == AESL_transaction_pc) {
          std::vector<sc_bv<16> > toScheduler_pc_buffer;
          int i = 0;
...
((char*)&xlx_stream_elt)[0*2+0] = toScheduler_pc_buffer[j].range(7, 0).to_int64();
((char*)&xlx_stream_elt)[0*2+1] = toScheduler_pc_buffer[j].range(15, 8).to_int64();
((hls::stream<__cosim_s2__>*)__xlx_apatb_param_toScheduler)->write(xlx_stream_elt);
}
}
        } // end transaction
      } // end file is good
...
transaction<16> tr(128);
  __xlx_offset_byte_param_realTaskId = 0*2;
  if (__xlx_apatb_param_realTaskId) {
tr.import<2>((char*)__xlx_apatb_param_realTaskId, 128, 0);
  }
...
unsigned char *pos = (unsigned char*)__xlx_apatb_param_realTaskId + i * 2;
...
#endif
std::vector<__cosim_s2__> __xlx_apatb_param_toScheduler_stream_buf;
long __xlx_apatb_param_toScheduler_stream_buf_size = ((hls::stream<__cosim_s2__>*)__xlx_apatb_param_toScheduler)->size();
#ifdef USE_BINARY_TV_FILE
{
...
long __xlx_apatb_param_toScheduler_stream_buf_final_size = ((hls::stream<__cosim_s2__>*)__xlx_apatb_param_toScheduler)->size() - __xlx_apatb_param_toScheduler_stream_buf_size;
{
  while (!((hls::stream<__cosim_s2__>*)__xlx_apatb_param_toScheduler)->empty())
    __xlx_apatb_param_toScheduler_stream_buf.push_back(((hls::stream<__cosim_s2__>*)__xlx_apatb_param_toScheduler)->read());
  for (int i = 0; i < __xlx_apatb_param_toScheduler_stream_buf.size(); ++i)
    ((hls::stream<__cosim_s2__>*)__xlx_apatb_param_toScheduler)->write(__xlx_apatb_param_toScheduler_stream_buf[i]);
  }
