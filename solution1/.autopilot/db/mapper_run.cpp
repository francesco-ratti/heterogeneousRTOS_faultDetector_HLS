...
extern "C" void run(__cosim_s32__*, __cosim_s4__, int*, short*, short*, int, __cosim_s2__*);
extern "C" void apatb_run_hw(__cosim_s4__* __xlx_apatb_param_contr, volatile void * __xlx_apatb_param_trainedRegions, volatile void * __xlx_apatb_param_realTaskId, volatile void * __xlx_apatb_param_n_regions_in, volatile void * __xlx_apatb_param_sharedMem, volatile void * __xlx_apatb_param_toScheduler) {
  ...
  // Collect __xlx_realTaskId__tmp_vec
  vector<sc_bv<16> >__xlx_realTaskId__tmp_vec;
  for (int j = 0, e = 128; j != e; ++j) {
    sc_bv<16> _xlx_tmp_sc;
    _xlx_tmp_sc.range(7, 0) = ((char*)__xlx_apatb_param_realTaskId)[j*2+0];
    _xlx_tmp_sc.range(15, 8) = ((char*)__xlx_apatb_param_realTaskId)[j*2+1];
    __xlx_realTaskId__tmp_vec.push_back(_xlx_tmp_sc);
  }
  int __xlx_size_param_realTaskId = 128;
  int __xlx_offset_param_realTaskId = 0;
  int __xlx_offset_byte_param_realTaskId = 0*2;
  short* __xlx_realTaskId__input_buffer= new short[__xlx_realTaskId__tmp_vec.size()];
  for (int i = 0; i < __xlx_realTaskId__tmp_vec.size(); ++i) {
    __xlx_realTaskId__input_buffer[i] = __xlx_realTaskId__tmp_vec[i].range(15, 0).to_uint64();
  }
  ...
  __cosim_s2__* __xlx_toScheduler_input_buffer= new __cosim_s2__[ap_apatb_toScheduler_cap_bc];
auto* stoScheduler = bcsim::createStream((hls::stream<__cosim_s2__>*)__xlx_apatb_param_toScheduler);
  // DUT call
  run(__xlx_sharedMem__input_buffer, *__xlx_apatb_param_contr, __xlx_trainedRegions__input_buffer, __xlx_realTaskId__input_buffer, __xlx_n_regions_in__input_buffer, __xlx_offset_byte_param_sharedMem, stoScheduler->data<__cosim_s2__>());
...
// print __xlx_apatb_param_realTaskId
  sc_bv<16>*__xlx_realTaskId_output_buffer = new sc_bv<16>[__xlx_size_param_realTaskId];
  for (int i = 0; i < __xlx_size_param_realTaskId; ++i) {
    __xlx_realTaskId_output_buffer[i] = __xlx_realTaskId__input_buffer[i+__xlx_offset_param_realTaskId];
  }
  for (int i = 0; i < __xlx_size_param_realTaskId; ++i) {
    ((char*)__xlx_apatb_param_realTaskId)[i*2+0] = __xlx_realTaskId_output_buffer[i].range(7, 0).to_uint();
    ((char*)__xlx_apatb_param_realTaskId)[i*2+1] = __xlx_realTaskId_output_buffer[i].range(15, 8).to_uint();
  }
...
stoScheduler->transfer((hls::stream<__cosim_s2__>*)__xlx_apatb_param_toScheduler);
}
