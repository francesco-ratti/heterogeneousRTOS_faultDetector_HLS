...
char FAULTDETECTOR_isFault(XRun *InstancePtr, u8 taskId) {
	return *((char*) (InstancePtr->Control_BaseAddress+XRUN_CONTROL_ADDR_ERRORINTASK_BASE+taskId*(XRUN_CONTROL_WIDTH_ERRORINTASK/8)));
}

void FAULTDETECTOR_resetFault(XRun *InstancePtr, u8 taskId) {
	*((char*) (InstancePtr->Control_BaseAddress+XRUN_CONTROL_ADDR_ERRORINTASK_BASE+taskId*(XRUN_CONTROL_WIDTH_ERRORINTASK/8)))=0;
}
